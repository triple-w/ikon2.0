        ...
        <div class="card-body">
            ...
            <ul class="permission-list">
                ...
                <?php if ($login_user->is_admin) { ?>
                    <li><span data-feather="shield" class="icon-14 ml-20"></span><h5><?php echo app_lang('fiscal_permissions'); ?>:</h5><?php foreach(['can_view_fiscal_profiles','can_manage_fiscal_profiles','can_view_fiscal_tax_settings','can_manage_fiscal_tax_settings','fiscal_items_view','fiscal_items_manage','fiscal_issuers_view','fiscal_issuers_manage','fiscal_series_view','fiscal_series_manage','fiscal_sales_review','fiscal_sales_pricing_review','fiscal_sales_pricing_apply','fiscal_sales_pricing_override','fiscal_drafts_view','fiscal_drafts_create','fiscal_drafts_lock','fiscal_drafts_supersede','fiscal_drafts_cancel','fiscal_xml_preview_generate','fiscal_xml_preview_view','fiscal_xml_preview_download','fiscal_xml_preview_validate','fiscal_certificates_view','fiscal_certificates_manage','fiscal_xml_sign','fiscal_signed_xml_view','fiscal_pac_status_view','fiscal_stamp_sandbox','fiscal_stamp_status','fiscal_stamp_reconcile','fiscal_stamped_xml_view','fiscal_stamped_xml_download','fiscal_stamp_error_details','fiscal_pdf_view','fiscal_pdf_download','fiscal_pdf_generate','fiscal_pdf_templates_view','fiscal_pdf_templates_manage','fiscal_invoices_view','fiscal_invoice_view','fiscal_xml_download','fiscal_cancel_request','fiscal_cancellation_receipt_view','fiscal_status_query','fiscal_invoices_download_xml','fiscal_invoices_download_pdf','fiscal_invoices_cancel','fiscal_invoices_view_cancellation','fiscal_invoices_reconcile_cancellation'] as $permission){ ?><div><?php echo form_checkbox($permission,'1',${$permission}?true:false,"id='$permission' class='form-check-input'"); ?><label for="<?php echo $permission; ?>"><?php echo app_lang($permission); ?></label></div><?php } ?><?php foreach(['fiscal.sales.invoice','fiscal.drafts.view','fiscal.drafts.create','fiscal.drafts.edit','fiscal.drafts.discard','fiscal.invoices.stamp','fiscal.invoices.view','fiscal.invoices.download_xml','fiscal.invoices.download_pdf','fiscal.invoices.send','fiscal.invoices.cancel','fiscal.advanced.view','fiscal.advanced.reconcile','fiscal.advanced.regenerate_pdf'] as $permission){ ?><div><?php echo form_checkbox($permission,'1',(bool)get_array_value($permissions,$permission),"id='".str_replace('.','_',$permission)."' class='form-check-input'"); ?><label for="<?php echo str_replace('.','_',$permission); ?>"><?php echo esc($permission); ?></label></div><?php } ?></li>
                    <li><span data-feather="briefcase" class="icon-14 ml-20"></span><h5>Permisos comerciales:</h5><?php foreach(['quotations.view','quotations.create','quotations.edit','quotations.send','quotations.accept','quotations.reject','quotations.cancel','quotations.convert','sales.view','sales.create','sales.edit','sales.close','sales.cancel','sales.edit_open','sales.override_close_validation'] as $permission){ ?><div><?php echo form_checkbox($permission,'1',(bool)get_array_value($permissions,$permission),"id='".str_replace('.','_',$permission)."' class='form-check-input'"); ?><label for="<?php echo str_replace('.','_',$permission); ?>"><?php echo esc($permission); ?></label></div><?php } ?><div class="text-off">sales.reopen reservado; no habilitado.</div></li>
                    <li>
                        <span data-feather="key" class="icon-14 ml-20"></span>

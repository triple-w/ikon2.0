            ...
            <div id="page-content" class="page-wrapper clearfix">
                ...
                <div class="invoice-preview estimate-preview">
                    <div class = "card  p15 no-border">
                        <div class="clearfix">
                            <?php
                            $final_statuses = array("accepted", "converted", "declined", "rejected", "cancelled", "expired");
                            if (in_array($estimate_info->status, $final_statuses, true)) {
                            ?>
                                <img class="dashboard-image float-start max-height-width-logo" src="<?php echo get_logo_url(); ?>" />
                                <div class="float-end mt10 mr15">
                                    <?php if ($estimate_info->status === "converted") { ?>
                                        <i data-feather="check-circle" class="icon-16 text-success"></i> <?php echo app_lang("estimate_accepted_and_converted"); ?>
                                    <?php } else if ($estimate_info->status === "accepted") { ?>
                                        <i data-feather="check-circle" class="icon-16 text-success"></i> <?php echo app_lang("estimate_accepted"); ?>
                                    <?php } else if ($estimate_info->status === "cancelled") { ?>
                                        <i data-feather="x-circle" class="icon-16 text-danger"></i> <?php echo app_lang("estimate_cancelled"); ?>
                                    <?php } else { ?>

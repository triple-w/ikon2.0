...
<?php if(!empty($can_view_supplier_costs)): ?><div class="col-md-12 mt15"><hr><div class="d-flex justify-content-between align-items-center"><h5>Proveedores y costos</h5><?php if(!empty($can_edit_supplier_costs))echo modal_anchor(get_uri('supplier_costs/form'),'<i data-feather="plus" class="icon-16"></i> Registrar costo',['class'=>'btn btn-primary btn-sm','title'=>'Registrar costo manual','data-post-product_id'=>$model_info->id]); ?></div><div class="row mb10"><div class="col-md-3"><small>Último costo</small><br><b><?php echo isset($supplier_cost_indicators['last_cost'])?to_currency($supplier_cost_indicators['last_cost']):'-'; ?></b></div><div class="col-md-3"><small>Último proveedor</small><br><b><?php echo esc($supplier_cost_indicators['last_supplier']??'-'); ?></b></div><div class="col-md-3"><small>Mejor histórico</small><br><b><?php echo isset($supplier_cost_indicators['best_cost'])?to_currency($supplier_cost_indicators['best_cost']):'-'; ?></b></div><div class="col-md-3"><small>Proveedores</small><br><b><?php echo(int)($supplier_cost_indicators['supplier_count']??0); ?></b></div></div><?php if(!empty($supplier_cost_summary)): ?><table class="table table-hover"><thead><tr><th>Proveedor</th><th>Último costo</th><th>Última fecha</th><th>Veces cotizado</th></tr></thead><tbody><?php foreach($supplier_cost_summary as$r): ?><tr><td><a href="<?php echo get_uri('suppliers/view/'.$r->supplier_id); ?>"><?php echo esc($r->supplier_name); ?></a></td><td><?php echo to_currency($r->last_cost); ?></td><td><?php echo esc(format_to_date($r->last_date,false)); ?></td><td><?php echo(int)$r->quote_count; ?></td></tr><?php endforeach; ?></tbody></table><details><summary>Ver historial de costos</summary><table class="table mt10"><thead><tr><th>Fecha</th><th>Proveedor</th><th>Costo</th><th>Cotización</th><th>Cliente</th><th>Precio venta</th></tr></thead><tbody><?php foreach(($supplier_cost_history??[])as$h): ?><tr><td><?php echo esc(format_to_date($h->quoted_at,false)); ?></td><td><a href="<?php echo get_uri('suppliers/view/'.$h->supplier_id); ?>"><?php echo esc($h->supplier_name); ?></a></td><td><?php echo to_currency($h->unit_cost); ?></td><td><?php if($h->source_type==='manual'): ?><span class="badge bg-info">Manual</span><?php else: ?><span><?php echo esc($h->document_folio); ?></span><?php endif; ?></td><td><?php echo esc($h->company_name?:'-'); ?></td><td><?php echo $h->sale_unit_price!==null?to_currency($h->sale_unit_price):'-'; ?></td></tr><?php endforeach; ?></tbody></table></details><?php else: ?><div class="alert alert-info">No existe historial comercial formalizado para este producto. Las propuestas en borrador no generan registros históricos.</div><?php endif; ?></div><?php endif; ?><div class="modal-footer">
    <?php

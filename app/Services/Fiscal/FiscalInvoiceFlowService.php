...
{
    public function __construct(private mixed $db=null,private ?FiscalDraftWorkflowService $workflow=null,private ?FiscalDraftStampingPreflightService $preflight=null,private ?FiscalDraftStampingService $stamping=null,private ?FiscalStampAccountService $wallet=null)
    {$this->db??=db_connect();$this->workflow??=new FiscalDraftWorkflowService($this->db);$this->preflight??=new FiscalDraftStampingPreflightService($this->db);$this->stamping??=new FiscalDraftStampingService($this->db);$this->wallet??=new FiscalStampAccountService($this->db);}

...
final class FiscalCancellationService
{
    ...
    public function cancel(int $documentId,string $reason,?string $replacementUuid,int $userId,bool $authorized):array
    {
        ...
        try{
            ...
            $payload=['uuid'=>$stamp->uuid,'reason'=>$reason,'replacement_uuid'=>$replacementUuid,'issuer_rfc'=>$issuer->rfc,'receiver_rfc'=>$receiver->rfc,'total'=>number_format((float)$document->total,6,'.','')];if(!$isFake)$payload+=$csdPayload;
    ...
    public function queryStatus(int$documentId,int$userId,bool$authorized,string$queryKey):array
    {
        ...
        $result=($this->adapter??new TimbradorXpressCancellationAdapter())->query(['uuid'=>$request->uuid,'issuer_rfc'=>$request->issuer_rfc,'receiver_rfc'=>$request->receiver_rfc,'total'=>number_format((float)$request->total,6,'.','')]);

...
final class CommercialItemTaxDisplayService
{
 ...
 public function present(array$c,string$qty,string$unitPrice,?string$currencySymbol=null):array{if(empty($c['ready']))return['ready'=>false,'unit_base'=>$unitPrice,'base'=>FiscalDecimal::multiply($unitPrice,$qty),'taxes'=>'Configuración pendiente','total'=>FiscalDecimal::multiply($unitPrice,$qty)];$names=['001'=>'ISR','002'=>'IVA','003'=>'IEPS'];$parts=[];foreach((array)$c['calculated_taxes']as$t){$name=$names[$t['tax_code']]??$t['tax_code'];$prefix=$t['tax_type']==='withholding'?'Retención ':'';$rate=$t['factor_type']==='Tasa'?(' '.rtrim(rtrim(number_format((float)$t['rate_or_quota']*100,6,'.',''),'0'),'.').'%'):(' '.$t['factor_type']);$sign=$t['tax_type']==='withholding'?'-':'';$parts[]=$prefix.$name.$rate.' · '.($currencySymbol===null?$sign.number_format((float)$t['tax_amount'],2,'.',','):to_currency($sign.$t['tax_amount'],$currencySymbol));}if(!$parts)$parts[]='Sin impuesto';return['ready'=>true,'unit_base'=>FiscalDecimal::divide($c['base'],$qty),'base'=>$c['base'],'taxes'=>implode('<br>',$parts),'total'=>$c['total']];}

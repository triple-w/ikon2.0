...
final class CfdiSemanticValidator{
 ...
 public function validate(CfdiDocument$c):array{$errors=[];$warnings=['La validación completa permanece pendiente de CSD, certificado y sello.'];$checks=[];$need=function(bool$ok,string$group,string$message)use(&$errors,&$checks){$checks[]=['group'=>$group,'message'=>$message,'passed'=>$ok];if(!$ok)$errors[$group][]=$message;};
  ...
  foreach($c->concepts as$n=>$concept){foreach(['product_service_code','quantity','unit_code','description','unit_value','gross_amount','tax_object_code']as$f)$need(trim((string)$concept->get($f))!=='','Conceptos',"Concepto ".($n+1).": falta $f.");$need($this->d->compare((string)$concept->get('quantity'),'0')>0,'Conceptos',"Concepto ".($n+1).': cantidad no positiva.');$subtotal=$this->d->add($subtotal,(string)$concept->get('gross_amount'));$discount=$this->d->add($discount,(string)$concept->get('discount'));
   foreach($concept->taxes as$t){$need($this->d->compare($t->base,'0')>=0,'Impuestos','La base no puede ser negativa.');$need(in_array($t->factorType,['Tasa','Cuota','Exento'],true),'Impuestos','TipoFactor inválido.');if($t->factorType!=='Exento')$need($t->rateOrQuota!==null,'Impuestos','Falta TasaOCuota.');if($t->taxType==='withheld')$withheld=$this->d->add($withheld,$t->amount);else$trans=$this->d->add($trans,$t->amount);$key=implode('|',[$t->taxCode,$t->taxType,$t->factorType,$t->rateOrQuota??'']);$summary[$key]['base']=$this->d->add($summary[$key]['base']??'0',$t->base);$summary[$key]['amount']=$this->d->add($summary[$key]['amount']??'0',$t->amount);}
  }
  $need($this->d->money($subtotal)===$this->d->money((string)$c->get('subtotal')),'Totales','La suma de conceptos no coincide con SubTotal.');$need($this->d->money($discount)===$this->d->money((string)$c->get('discount')),'Totales','La suma de descuentos no coincide.');$need($this->d->money($trans)===$this->d->money((string)$c->get('transferred_tax_total')),'Impuestos','Los traslados por concepto no coinciden.');$need($this->d->money($withheld)===$this->d->money((string)$c->get('withheld_tax_total')),'Impuestos','Las retenciones por concepto no coinciden.');
  ...
  $calc=$this->d->money($this->d->sub($this->d->add($this->d->sub($subtotal,$discount),$trans),$withheld));$need($calc===$this->d->money((string)$c->get('total')),'Totales','SubTotal - Descuento + Traslados - Retenciones no coincide con Total.');

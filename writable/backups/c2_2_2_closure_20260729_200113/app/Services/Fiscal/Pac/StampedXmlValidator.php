...
final class StampedXmlValidator
{
    ...
    public function validate(string $stampedXml,string $signedXml): array
    {
        ...
        if(!hash_equals($signed->documentElement->getAttribute('Sello'),$tfd->getAttribute('SelloCFD')))throw new RuntimeException('SelloCFD no coincide con el sello local enviado.');
    ...
    {
        foreach(['Version','Serie','Folio','SubTotal','Descuento','Total','Moneda','TipoDeComprobante','Sello']as$n)if($a->getAttribute($n)!==$b->getAttribute($n))throw new RuntimeException("El PAC modificó el atributo {$n}.");

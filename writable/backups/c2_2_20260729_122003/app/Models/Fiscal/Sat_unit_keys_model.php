...
class Sat_unit_keys_model extends Sat_catalog_model {
    ...
    public function search(string $term,int $page=1,int $limit=20):array{
        ...
        $rank="CASE WHEN code = $exact THEN 0 WHEN code LIKE $prefix THEN 1 WHEN name LIKE $prefix OR description LIKE $prefix THEN 2 ELSE 3 END";
        $builder=$this->db->table($this->table)->select("id,code,name,description,$rank AS search_rank",false)->where('is_active',1)->groupStart()->like('code',$term)->orLike('name',$term)->orLike('description',$term)->groupEnd()->orderBy('search_rank')->orderBy('code')->limit($limit+1,($page-1)*$limit);
